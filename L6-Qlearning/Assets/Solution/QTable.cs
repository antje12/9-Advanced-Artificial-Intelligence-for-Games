...
namespace Solution
{
    public class QTable
    {
        ...
        public void updateQvalue(int reward, char[] map)
        {
            ...
            float[] prevQValues = getActionsQValues(prevState);
            float prevQValueForAction = prevQValues[prevAction];

            float newQValue = prevQValueForAction + learningRate * (reward + gammaValue * bestValue - prevQValueForAction);
            table[getMapString(prevState)][prevAction] = newQValue;
        }
